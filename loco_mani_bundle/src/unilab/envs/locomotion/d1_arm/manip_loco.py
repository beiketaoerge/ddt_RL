...
from unilab.envs.locomotion.d1_arm.base import (
    ARM_IDX,
    DEFAULT_LEG_ANGLES,
    LEG_POS_IDX,
    WHEEL_VEL_IDX,
    ...
    D1ArmBaseEnv,
    D1ArmSensor,
    build_d1_arm_position_gains,
)


def _default_d1_arm_model_file() -> str:
    return str(ASSETS_ROOT_PATH / "robots" / "d1_arm" / "scene_flat.xml")


def _default_d1_arm_scene() -> SceneCfg:
    return SceneCfg(model_file=_default_d1_arm_model_file())


def _resolve_d1_arm_scene(cfg: "D1ArmManipLocoCfg") -> SceneCfg:
    scene = cfg.scene
    default_model_file = _default_d1_arm_model_file()
    if scene is None:
...
@dataclass
class InitState:
    pos: list[float] = field(default_factory=lambda: [0.0, 0.0, 0.60])


@dataclass
class D1ArmDomainRandConfig(DomainRandConfig):
...
@registry.envcfg("D1ArmManipLoco")
@dataclass
class D1ArmManipLocoCfg(D1ArmBaseCfg):
    scene: SceneCfg = field(default_factory=_default_d1_arm_scene)
    model_file: str = field(default_factory=_default_d1_arm_model_file)
    max_episode_seconds: float = 20.0
    init_state: InitState = field(default_factory=InitState)
    commands: CommandsConfig = field(default_factory=CommandsConfig)  # type: ignore[assignment]
    reward_config: RewardConfig | None = None
    sensor: D1ArmSensor = field(default_factory=D1ArmSensor)  # type: ignore[assignment]
    domain_rand: D1ArmDomainRandConfig = field(default_factory=D1ArmDomainRandConfig)
...
class D1ArmManipLocoDRProvider(LocomotionDRProvider):
    def __init__(
        self,
        *,
...
@registry.env("D1ArmManipLoco", sim_backend="motrix")
@registry.env("D1ArmManipLoco", sim_backend="mujoco")
class D1ArmManipLocoEnv(D1ArmBaseEnv):
    _cfg: D1ArmManipLocoCfg

    def __init__(self, cfg: D1ArmManipLocoCfg, num_envs=1, backend_type="mujoco"):
        if cfg.reward_config is None:
            raise ValueError("reward_config must be provided via Hydra configuration")
        if backend_type not in {"mujoco", "motrix"}:
            raise ValueError(
                "D1ArmManipLoco supports only the mujoco and motrix backends, "
                f"got {backend_type!r}"
            )

        scene = _resolve_d1_arm_scene(cfg)
        ...
        if backend_type == "motrix":
            ...
            backend_kwargs["position_actuator_gains"] = build_d1_arm_position_gains(
                cfg.control_config
            )
        ...
        if self._num_action != 22:
            raise ValueError(f"D1ArmManipLoco expects 22 actuators, got {self._num_action}")
        ...
        #               diff(22)+dof_vel(22)+ee_local_pos(3)+ee_goal_cart(3)+
        #               ee_error(3)+last_actions(22) = 91 dims.
        _CRITIC_ONE = 91  # Single-step critic obs dim, including privileged linvel.
        _ACTOR_ONE = 88  # Single-step actor obs dim after removing linvel[0:3].
        ...
        dr_provider = D1ArmManipLocoDRProvider(
            base_kp=base_kp,
            base_kd=base_kd,
    ...
    def apply_action(self, actions: np.ndarray, state: NpEnvState) -> np.ndarray:
        # actions shape: (num_envs, 22), matching qpos/actuator order:
        # per-leg [hip_pos, thigh_pos, calf_pos, wheel_vel] x 4, then arm x 6.
        state.info["last_actions"] = state.info.get("current_actions", np.zeros_like(actions))
        stage_cfg = self._cfg.arm_stage
        if stage_cfg.freeze_arm_joints:
            effective_actions = actions.copy()
            effective_actions[:, ARM_IDX] = 0.0
        else:
        ...
        ctrl[:, WHEEL_VEL_IDX] = (
            exec_actions[:, WHEEL_VEL_IDX] * self._cfg.control_config.wheel_action_scale
        )
        # Arm position control: 6 joints at [16:22]
        if stage_cfg.freeze_arm_joints:
            ctrl[:, ARM_IDX] = np.broadcast_to(
                self.default_angles[ARM_IDX], (self._num_envs, 6)
            ).astype(get_global_dtype(), copy=False)
        else:
            ctrl[:, ARM_IDX] = (
                self.get_arm_dof_pos()
                + exec_actions[:, ARM_IDX] * self._cfg.control_config.arm_action_scale
    ...
    def _init_reward_functions(self) -> None:
        self._reward_fns: dict[str, Any] = {
            ...
            "torques": rewards.torques,  # L1 torque over all 22 DOFs.
            "energy": rewards.energy,  # Requires ctx.dof_vel and info["torques"].
            "dof_vel": self._reward_dof_vel,  # L2 velocity over all 22 DOFs.
    ...
    def _compute_raw_obs(
        ...
        *,
        ...
        """Compute single-step 91-dim obs (no history).

        Layout: linvel(3)+gyro(3)+(-gravity)(3)+command(3)+feet_phase(4)+
                diff(22)+dof_vel(22)+ee_local_pos(3)+ee_goal_cart(3)+ee_error(3)+
                last_actions(22) = 91
        """
    ...
    def _update_history(
        ...
        *,
        ...
        Actor buffer stores obs WITHOUT linvel (raw_obs[:, 3:], 88-dim) so the actor
        cannot shortcut the estimator. Critic buffer stores clean full 91-dim obs
        when a separate critic_raw_obs is provided.
        """
        A = self._actor_one_step_dim  # 88
        C = self._critic_one_step_dim  # 91
        H_a = self._cfg.history.num_actor_history
        H_c = self._cfg.history.num_critic_history
    ...
    def _reward_stand_still(self, ctx: RewardContext) -> np.ndarray:
        """Penalize leg-position-joint deviation from default when command is near zero.

        Uses LEG_POS_IDX to select only the 12 position-controlled leg joints,
        skipping wheel and arm joints.
        """
        ...
        dof_error = np.sum(
            np.abs(ctx.dof_pos[:, LEG_POS_IDX] - ctx.default_angles[LEG_POS_IDX]), axis=1
        )
        return is_still * dof_error

    def _reward_dof_vel(self, ctx: RewardContext) -> np.ndarray:
        """L2 velocity penalty over all 22 joints."""
    ...
    def _reward_dof_pos_limits(self, ctx: RewardContext) -> np.ndarray:
        """Leg soft-limit penalty on position-controlled joints only."""
        cfg = self._reward_cfg
        ...
        leg_pos = ctx.dof_pos[:, LEG_POS_IDX]
